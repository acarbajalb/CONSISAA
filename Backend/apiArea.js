...
router.post("/creacionArea", async (req, res) => {
  try {
    ...
    db.query(
      ...
      (err, data) => {
        if (err) {
          console.error("Error al insertar área:", err);
        ...
        registrarEnBitacora(164, "Crear área", "Se creó un nueva área.");

        return res.json({ message: "Área insertada correctamente" });//Mensaje que se manda al frotn
      }
    );
  } catch (error) {
...
router.get("/traerArea", (req, res) => {
  ...
  db.query(query, (err, data) => {
      ...
      registrarEnBitacora(163, "Entró a las áreas", "Se mostraron los registros de las áreas.");
...
router.put("/editarArea/:id_area", async (req, res) => {
  try {
    ...
    db.query(query, values, (err, data) => {
      ...
      registrarEnBitacora(165, "Editar área", `Se editó el área con ID ${IdArea}.`);

      return res.json({ message: "Área actualizada correctamente" });
    });
  } catch (error) {
    console.error("Error al editar área:", error);
    return res.status(500).json({ error: "Error al editar área en la base de datos" });
  }
});
