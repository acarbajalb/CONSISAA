...
async function insertarEmpleadoYTelefono(Id_Estado,Id_EstadoCivil, Id_Genero, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Numero, Id_Tipo_Telefono, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion) {
    return new Promise((resolve, reject) => {
        db.beginTransaction(async (err) => {
            ...
            try {
                ...
                const empleadoQuery = 'INSERT INTO tbl_empleado (Id_Estado,Id_EstadoCivil, Id_Genero, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion) VALUES (?,?,?,?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)';
                const empleadoResult = await executeQuery(empleadoQuery, [Id_Estado,Id_EstadoCivil, Id_Genero, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion]);

                const empleadoId = empleadoResult.insertId;

                // Insertar en tbl_telefono_empleado
                const telefonoQuery = 'INSERT INTO tbl_telefono_empleado (Id_empleado, Id_tipo_telefono, Numero, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)';
                await executeQuery(telefonoQuery, [empleadoId, Id_Tipo_Telefono, Numero, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion]);
...
router.post("/crearEmpleado", async (req, res) => {
    try {
        ...
        await insertarEmpleadoYTelefono( Id_Estado,Id_EstadoCivil, Id_Genero, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Numero, Id_Tipo_Telefono, Creado_por, Modificado_por, Fecha_creacion, Fecha_modificacion);
        
        registrarEnBitacora( 6, "Creo Empleado", "Se creo un nuevo empleado");
...
`;
    db.query(query, (err, data) => {
        ...
        registrarEnBitacora( 5, "Obtener Empleados", "Se obtuvieron los Empleados");
...
async function actualizarEmpleadoYTelefono(Id_Empleado,Id_Estado,Id_Genero,Id_EstadoCivil,Id_tipo_telefono, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Numero, Modificado_por) {
  return new Promise((resolve, reject) => {
      db.beginTransaction(async (err) => {
          if (err) {
              return reject(err);
          ...
          try {
              ...
              const empleadoQuery = 'UPDATE tbl_empleado SET Id_Estado =?,Id_Genero =? ,Id_EstadoCivil= ? ,Nombre = ?, Apellido = ?, Dni = ? ,Direccion_vivienda = ?, Salario = ?, Modificado_por = ?, Fecha_modificacion = CURRENT_TIMESTAMP WHERE Id_empleado = ?';
              await executeQuery(empleadoQuery, [Id_Estado,Id_Genero,Id_EstadoCivil,Nombre, Apellido,Dni, Direccion_vivienda, Salario, Modificado_por, Id_Empleado]);

              // Actualizar en tbl_telefono_empleado
              const telefonoQuery = 'UPDATE tbl_telefono_empleado SET Id_tipo_telefono= ? , Numero = ?, Modificado_por = ?, Fecha_modificacion = CURRENT_TIMESTAMP WHERE Id_empleado = ?';
              await executeQuery(telefonoQuery, [Id_tipo_telefono,Numero, Modificado_por, Id_Empleado]);
...
router.put("/editarEmpleado/:Id_Empleado", async (req, res) => {
  try {
      ...
      await actualizarEmpleadoYTelefono(Id_Empleado,Id_Estado,Id_Genero,Id_EstadoCivil,Id_tipo_telefono, Nombre, Apellido,Dni, Direccion_vivienda, Salario, Numero, Modificado_por);

    
     registrarEnBitacora( 7, "Editar Empleado", `Se editó al empleado con ID ${Id_Empleado}`);
...
router.put("/deshabilitarEmpleado/:Id_Empleado", async (req, res) => {
    try {
        ...
        db.query(query, [empleadoId], (err, result) => {
            if (err) {
                ...
                return res.status(500).json({ error: "Error al eliminar empleado en la base de datos" });
            }
            if (result.affectedRows === 0) {
                return res.status(404).json({ error: "Empleado no encontrado para eliminar" });
            }

            registrarEnBitacora( 12, "Deshabilitar empleado", `Se deshabilitó el empleado con ID ${empleadoId}`);
 ...
 //SELECT A OTROS DATOS DE OTRAS TABLAS NECESARIAS PARA EL CRUD EMPLEADO
  ...
  // Endpoint para obtener Tipos de telefono
