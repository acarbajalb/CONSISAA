...
const CrearEmpleado = ({ onClose }) => {
  ...
  const creacionEmpleado = async (event) => {
    ...
    try {
      ...
      const result = await response.json(); // Obtener el contenido de la respuesta en formato JSON
      ...
      } else if (result.error === 'DNI ya registrada. Ingresa datos nuevos.') {         
      ...
      <div className="input-container">
        <div className="input-group">
          <input
            ...
            maxLength={100}
            minLength={8}
            style={{ width: "900px", height: "80px", marginTop: "15px" }}
            required
            onInput={(e) => {
              // Obtener el valor del input
              let value = e.target.value;
