...
import "./nuevoEstado.css";
import Swal from "sweetalert2";

const NuevoEstado = ({ onClose }) => {
  ...
  const creacionEstado = async (event) => {
    event.preventDefault();

    try {
      const response = await fetch("http://localhost:3001/creacionEstado", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        Toast.fire({
          icon: "success",
          title: "Estado  Creado",
          text: result.message || "El estado  ha sido creado con éxito",
        });
  ...
  return (
    <form onSubmit={creacionEstado} className="Crear" Nuevo-Estado="true">
      <h1 id="Titulo">CREAR ESTADO</h1>
