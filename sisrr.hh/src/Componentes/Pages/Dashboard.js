...
const Dashboard = () => {
  ...
  const [nombre, setNombre] = useState("CONSISA"); // Estado para almacenar el nombre obtenido
  ...
  useEffect(() => {
    ...
    // Función para obtener el nombre del parámetro
    const obtenerNombreParametro = async () => {
      try {
        const response = await fetch(`http://localhost:3001/obtenerNombre`);
        const data = await response.json();
        if (response.ok) {
          setNombre(data.Valor);
        } else {
          console.error("Error al obtener el valor del parámetro:", data.message);
        }
      } catch (error) {
  ...
  return (
    <div className="dashboard">
      <nav className={`sidebar ${isSidebarClosed ? "close" : ""}`}>
        <header>
          <div className="image-text">
            ...
            <span className="name">{nombre}</span> {/* Mostrar el nombre obtenido */}
              ...
              <li className="nav-link with-submenu">
                ...
                {isSidebarClosed ? null : (
                  <ul className="Seguridad">
                    ...
                    <li>
                      <a href="#">
                      <i className="bx bx-cloud-download icon"></i>
                        <span
                          className="text nav-text"
                          onClick={() => handleComponentToggle("BackupRestore")}
                        >
      ...
      <section className="home">
        ...
        {/*Mantenimiento Backup y restore*/}
     {activeComponent === "BackupRestore" && (
          <div className="text">
            <BackupRestore onClose={() => setActiveComponent(null)} />
          </div>
        )}
